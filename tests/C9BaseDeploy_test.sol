...
contract C9TestContract is C9Struct {
    ...
    uint256 constant DATA_SIZE = 16;
    TokenData[DATA_SIZE] internal _rawData;
    ...
    constructor() {
        ...
        _rawData[11] = TokenData("SPLASH", 0, 0, 0, 0, 0, 5, 4, 0, 1, 1, 0, 2, 0, 35, 0, 78077, 0, 0, 2500, 10, 5799277778962795249624098336453211151371729106509768896000264210351389);
        _rawData[12] = TokenData("SPOOK", 0, 0, 0, 0, 0, 5, 1, 3, 1, 4, 0, 5, 0, 47, 0, 72535, 0, 0, 350, 2, 5799134626889714050492983678154145488816434763104877355262923114034857);
        _rawData[13] = TokenData("GOLDIE", 0, 0, 0, 0, 0, 6, 1, 2, 1, 0, 0, 5, 0, 47, 0, 77003, 0, 0, 300, 2, 5799241467364358421928659436044716419926730545198957899099418689026081);
        _rawData[14] = TokenData("TEDDY OF CRANBERRY", 0, 0, 0, 0, 0, 2, 4, 2, 1, 5, 0, 5, 0, 42, 0, 76090, 0, 0, 750, 2, 5799282743624756829579178850235960144078557193791151690439773890808985);
        _rawData[15] = TokenData("ALLY", 0, 0, 0, 0, 0, 5, 1, 3, 1, 4, 0, 5, 0, 50, 0, 893379, 0, 0, 200, 2, 24322943308668699081434806879740272793683024287520481799047116943370226370997);
        //_rawData[16] = TokenData("TEDDY OF MAGENTA", 0, 0, 1, 0, 0, 6, 1, 2, 1, 0, 0, 5, 0, 45, 0, 78274, 0, 0, 500, 2, 5799040797830861292230812403166935521563202740289193265615463921626077);
        //_rawData[17] = TokenData("DERBY CM", 0, 0, 1, 0, 0, 6, 1, 3, 1, 0, 0, 6, 0, 50, 0, 76015, 0, 0, 125, 1, 5798952723946465783293057815368373566671191673007074447422925889605209);
        //_rawData[18] = TokenData("ALLY", 0, 0, 0, 0, 0, 2, 4, 2, 1, 0, 0, 4, 0, 45, 0, 69358, 0, 0, 500, 3, 5799208630858884874046788132325762798534203686929889537546296853407797);
        //_rawData[19] = TokenData("FLASH", 0, 0, 0, 4, 0, 6, 4, 1, 1, 1, 0, 4, 0, 37, 0, 894296, 0, 0, 1500, 3, 24323467797581229811197741915687639246135400876943092544792468055570397136869);
        ...
        TokenData[] memory minterData = new TokenData[](DATA_SIZE);
        for (uint256 i; i<DATA_SIZE;) {
            _rawData[i].validitystamp = _timestamp;
            minterData[i] = _rawData[i];
            tokenOwner[_rawData[i].tokenid] = c9tOwner; 
            unchecked {++i;}
        }
    ...
    internal {
        Assert.equal(c9t.balanceOf(_address), balances[_address], "X balanceOf");
        Assert.equal(c9t.transfersOf(_address), transfers[_address], "X transfersOf");
        Assert.equal(c9t.votesOf(_address), votes[_address], "X votesOf");
        Assert.equal(c9t.isRegistered(_address), regStatus, "X regStatus");
        Assert.equal(c9t.redemptionsOf(_address), redemptions[_address], "X redemptionsOf");
    }
    ...
    internal {
        ...
        Assert.equal(_tokenParams[0], _timestamp, "X mintstamp");
        Assert.equal(_tokenParams[3], rawdata.cntrytag, "X cntrytag");
        Assert.equal(_tokenParams[4], rawdata.cntrytush, "X cntrytush");
        Assert.equal(_tokenParams[5], rawdata.gentag, "X gentag");
        Assert.equal(_tokenParams[6], rawdata.gentush, "X gentush");
        Assert.equal(_tokenParams[7], rawdata.markertush, "X markertush");
        Assert.equal(_tokenParams[8], rawdata.special, "X special");
        Assert.equal(_tokenParams[9], rawdata.raritytier, "X raritytier");
        Assert.equal(_tokenParams[10], rawdata.royaltiesdue, "X royalties due");
    }
    ...
    internal {
        ...
        // actual, expected
        Assert.equal(_ownerParams[0], tokenTransfers[tokenId], "X xfer counter");
        Assert.equal(_ownerParams[1], rawdata.validitystamp, "X validitystamp");
        Assert.equal(_ownerParams[2], rawdata.validity, "X validity");
        Assert.equal(_ownerParams[3], rawdata.upgraded, "X upgraded");
        Assert.equal(_ownerParams[4], rawdata.display, "X display");
        Assert.equal(_ownerParams[5], rawdata.locked, "X locked");
        Assert.equal(_ownerParams[6], rawdata.insurance, "X insurance");
        Assert.equal(_ownerParams[7], rawdata.royalty*10, "X royalty");
        Assert.equal(_ownerParams[8], rawdata.votes, "X votes");
    }
    ...
    internal {
        ...
        Assert.equal(_islocked, rawdata.locked, "X view locked");

        uint256 _isupgraded = c9t.isUpgraded(tokenId) ? 1 : 0;
        Assert.equal(_isupgraded, rawdata.upgraded, "X view upgraded");

        Assert.equal(c9t.validityStatus(tokenId), rawdata.validity, "X view validity status");
        Assert.equal(c9t.ownerOf(tokenId), tokenOwner[tokenId], "X view owner");
        Assert.equal(c9t.getTokenParamsName(tokenId), rawdata.name, "X view name");

        (, uint256 _royaltyinfo) = c9t.royaltyInfo(tokenId, 10000);
        Assert.equal(_royaltyinfo, rawdata.royalty*10, "X view royalty info");

        Assert.equal(c9t.preRedeemable(tokenId), true, "X view preredeemable");
        Assert.equal(c9t.isRedeemable(tokenId), false, "X view redeemable");
        Assert.equal(c9t.isRedeemed(tokenId), tokenRedeemed[tokenId], "X view redeemaed");
    }

    function checkTotalSupply()
    public {
         Assert.equal(c9t.totalSupply(), DATA_SIZE, "X total supply");
    }
    ...
    internal {
        ...
        Assert.equal(validityStatus, status, "X validity status");
    ...
    public {
        Assert.equal(c9t.owner(), c9tOwner, "X owner");
        Assert.equal(c9t.totalSupply(), DATA_SIZE, "X supply");
    }
    ...
    public {
        ...
        Assert.equal(c9t.totalSupply(), mintSupply, "X mint counter supply");
    }
}