...
contract C9Market is C9OwnerControl {
    ...
    function _checkSignerSendToken(uint256 _tokenId, uint256 _listingPrice, bytes calldata sig)
        private {
    ...
    function purchaseToken(uint256 _tokenId, uint256 _listingPrice, bytes calldata sig)
        external payable
        notFrozen() {
            _checkSignerSendToken(_tokenId, _listingPrice, sig);
    ...
    function purchaseTokens(
        ...
        notFrozen() {
            ...
            for (uint256 i; i<_batchSize;) {
                _checkSignerSendToken(_tokenId[i], _listingPrice[i], sig[i]);
