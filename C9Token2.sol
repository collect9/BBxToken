...
contract C9Token is C9Struct, ERC721IdEnumBasic {
    /**
    ...
    notFrozen() {
        ...
        if ((_tokenData>>DPOS_LOCKED & BOOL_MASK) == LOCKED) {
    ...
    private pure
    returns (uint256) {
        return _viewPackedData(tokenData, DPOS_VALIDITY, DSZ_VALIDITY);
    }
    ...
    requireMinted(tokenId) {
        ...
        if (uint256(uint16(_tokenData>>DPOS_ROYALTIES_DUE)) == amount) {
            revert RoyaltiesAlreadySet();
        }
        _uTokenData[tokenId] = _setTokenParam(
            _tokenData,
    ...
    private {
        ...
        for (uint256 i; i<batchSize;) {
            ...
            packedToken |= timestamp<<XPOS_MINTSTAMP;
            packedToken |= edition<<XPOS_EDITION;
            packedToken |= editionMintId<<XPOS_EDITION_MINT_ID;
            packedToken |= _input.cntrytag<<XPOS_CNTRYTAG;
            packedToken |= _input.cntrytush<<XPOS_CNTRYTUSH;
            packedToken |= _input.gentag<<XPOS_GENTAG;
            packedToken |= _input.gentush<<XPOS_GENTUSH;
            packedToken |= _input.markertush<<XPOS_MARKERTUSH;
            packedToken |= _input.special<<XPOS_SPECIAL;
            packedToken |= _input.raritytier<<XPOS_RARITYTIER;
            _owners[tokenId] = packedToken;

            // Additional storage in _uTokenData
            uint256 tokenData;
            unchecked {
                tokenData = totalSupply() - batchSize + i + 1;
            }
            tokenData |= timestamp<<DPOS_VALIDITYSTAMP;
            tokenData |= _input.validity<<DPOS_VALIDITY;
            tokenData |= _input.upgraded<<DPOS_UPGRADED;
            tokenData |= _input.display<<DPOS_DISPLAY;
            tokenData |= _input.locked<<DPOS_LOCKED;
            tokenData |= _input.royalty<<DPOS_ROYALTY;
            tokenData |= _input.royaltiesdue<<DPOS_ROYALTIES_DUE;
            tokenData |= _input.insurance<<DPOS_INSURANCE;
            _uTokenData[tokenId] = tokenData;
    ...
    onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        _setTokenData(input);
        unchecked {
            _balances[_msgSender()] += tokenIds.length;
        }
    }
    ...
    private view
    returns (bool) {
        uint256 _ds = block.timestamp - uint256(uint40(tokenData>>XPOS_MINTSTAMP));
    ...
    private {
        ...
        _uTokenData[tokenId] = _tokenData;
    ...
    function _unlockToken(uint256 _tokenId)
        private {
            ...
            if ((_tokenData>>DPOS_LOCKED & BOOL_MASK) == UNLOCKED) {
    ...
    function burn(uint256 tokenId)
        ...
        isOwnerOrApproved(tokenId) {
            ...
            if (_msgSender() != owner) {
                ...
                uint256 _validityStamp = uint256(uint40(_tokenData>>DPOS_VALIDITYSTAMP));
    ...
    function getTokenParams(uint256 tokenId)
        ...
        returns(uint256[20] memory xParams) {
            uint256 ownersData = _owners[tokenId];
            uint256 uTokenData = _uTokenData[tokenId];

            xParams[0] = uint256(uint40(ownersData));
            xParams[1] = uint256(uint8(ownersData>>XPOS_EDITION));
            xParams[2] = uint256(uint16(ownersData>>XPOS_EDITION_MINT_ID));
            xParams[3] = _viewPackedData(ownersData, XPOS_CNTRYTAG, XSZ_CNTRYTAG);
            xParams[4] = _viewPackedData(ownersData, XPOS_CNTRYTUSH, XSZ_CNTRYTUSH);
            xParams[5] = _viewPackedData(ownersData, XPOS_GENTAG, XSZ_GENTAG);
            xParams[6] = _viewPackedData(ownersData, XPOS_GENTUSH, XSZ_GENTUSH);
            xParams[7] = _viewPackedData(ownersData, XPOS_MARKERTUSH, XSZ_MARKERTUSH);
            xParams[8] = _viewPackedData(ownersData, XPOS_SPECIAL, XSZ_SPECIAL);
            xParams[9] = _viewPackedData(ownersData, XPOS_RARITYTIER, XSZ_RARITYTIER);

            xParams[10] = uint256(uint24(uTokenData));
            xParams[11] = uint256(uint40(uTokenData>>DPOS_VALIDITYSTAMP));
            xParams[12] = _currentVId(uTokenData);

            xParams[13] = uTokenData>>DPOS_UPGRADED & BOOL_MASK;
            xParams[14] = uTokenData>>DPOS_DISPLAY & BOOL_MASK;
            xParams[15] = uTokenData>>DPOS_LOCKED & BOOL_MASK;

            xParams[16] = _viewPackedData(uTokenData, DPOS_ROYALTY, DSZ_ROYALTY);
            xParams[17] = uint256(uint16(uTokenData>>DPOS_ROYALTIES_DUE));
            xParams[18] = _viewPackedData(uTokenData, DPOS_INSURANCE, DSZ_INSURANCE);
            xParams[19] = uint256(uint24(uTokenData>>DPOS_XFER_COUNTER));
    }
    ...
    function setTokenDisplay(uint256 tokenId, bool flag)
        ...
        isOwnerOrApproved(tokenId) {
            ...
            uint256 _val = tokenData>>DPOS_UPGRADED & BOOL_MASK;
            if (_val != UPGRADED) {
                revert TokenNotUpgraded(tokenId);
            }
            _val = tokenData>>DPOS_DISPLAY & BOOL_MASK;
    ...
    function setTokenUpgraded(uint256 tokenId)
        ...
        notDead(tokenId) {
            uint256 _tokenData = _uTokenData[tokenId];
            if ((_tokenData>>DPOS_UPGRADED & BOOL_MASK) == UPGRADED) {
    ...
    function tokenURI(uint256 _tokenId)
        ...
        returns (string memory) {
            uint256 _tokenData = _uTokenData[_tokenId];
            bool _externalView = (_tokenData>>DPOS_DISPLAY & BOOL_MASK) == EXTERNAL_IMG;
            bytes memory image;
